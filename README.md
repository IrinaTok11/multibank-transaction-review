...
Bank statements from different banks don't share a layout. Some use separate Debit and Credit columns, others a single signed Amount, others "Paid in / Paid out". Without a script, each file has to be adjusted manually before the analysis starts. This script handles the normalisation and produces one flat table ready for the next step.
...
![Pipeline overview — raw multi-bank inputs to a standardised transaction table](docs/assets/pipeline_overview.png)
...
├─ docs/
│  └─ assets/
│     ├─ pipeline_overview.png         # pipeline diagram (inputs → merged output)
